# From the file content you provided:
...
bottom_layer: tuple[slice, ...] = (slice(None, None), slice(-1, None))
top_layer: tuple[slice, ...] = (slice(None), slice(0, 1))
upper_edges: tuple[slice, ...] = (slice(None, None), slice(1, None))
lower_edges: tuple[slice, ...] = (slice(None, None), slice(None, -1))
...
@Dimensionalize(  # <--- USE THE NEW BESPOKE DECORATOR HERE
    argument_dimensions=(
...
def RT_Toon1989(
    ...
    final_result_flux = calculate_flux(  # Call calculate_flux to get the result
        optical_depth,
        single_scattering_albedo,
        scattering_asymmetry_parameter,
        thermal_intensity,
...
@njit  # Keep DSolver_subroutine as njit, try parallel=True
def DSolver_subroutine(
    ...
    num_wavelengths = odd_afs.shape[0]
    num_odd_cols = odd_afs.shape[1]
    num_even_cols = even_afs.shape[1]

    interleaved_dimension_size = num_odd_cols + num_even_cols

    # Explicitly define the 2D shape tuple, where both elements are concrete ints.
    # This is the key fix for the tuple(list(int64)<iv=None>) error.
    final_interleaved_shape_tuple = (
        num_wavelengths,  # First dimension (wavelengths) is fixed as integer
        interleaved_dimension_size,  # Interleaved last dimension is fixed as integer
    )

    interleaved_afs = np.empty(final_interleaved_shape_tuple, dtype=odd_afs.dtype)
    interleaved_bfs = np.empty(final_interleaved_shape_tuple, dtype=odd_bfs.dtype)
    interleaved_cfs = np.empty(final_interleaved_shape_tuple, dtype=odd_cfs.dtype)
    interleaved_dfs = np.empty(final_interleaved_shape_tuple, dtype=odd_dfs.dtype)

    # 3. Manually add even and odd components for ALL FOUR ARRAYS in a single set of loops
    #    This replaces the four separate loop blocks.

    for i in range(num_wavelengths):  # Use prange for potential parallelization
        for j in range(num_odd_cols):
            # Assign to AFs, BFSs, CFSs, DFSs simultaneously for even positions
            interleaved_afs[i, 2 * j] = odd_afs[i, j]
            interleaved_bfs[i, 2 * j] = odd_bfs[i, j]
            interleaved_cfs[i, 2 * j] = odd_cfs[i, j]
            interleaved_dfs[i, 2 * j] = odd_dfs[i, j]

        for j in range(num_even_cols):
            # Assign to AFs, BFSs, CFSs, DFSs simultaneously for odd positions
            interleaved_afs[i, 2 * j + 1] = even_afs[i, j]
            interleaved_bfs[i, 2 * j + 1] = even_bfs[i, j]
            interleaved_cfs[i, 2 * j + 1] = even_cfs[i, j]
            interleaved_dfs[i, 2 * j + 1] = even_dfs[i, j]

    # --- END: Inlined interleave logic (combined loops) ---
...
def DTRIDGL_subroutine(
    ...
    as_terms: NDArray[np.float64] = np.zeros_like(afs, dtype=np.float64)
    as_terms[*bottom_layer] = as_base
    ds_terms: NDArray[np.float64] = np.zeros_like(afs, dtype=np.float64)
    ...
    for half_layer in range(twice_number_of_layers - 1, -1, -1):
    ...
    xki_terms: NDArray[np.float64] = np.zeros_like(ds_terms)
...
# --- Add this helper function (before calculate_flux) ---
@njit  # Apply njit to the loop kernel
def _calculate_flux_loop_kernel(
    # Inputs needed for the loop:
    initial_fpt_terms: NDArray[
        np.float64
    ],  # This is the fpt_terms array that gets updated
    ...
    epp_terms: NDArray[np.float64],
    em2_terms: NDArray[np.float64],
    em3_terms: NDArray[np.float64],
    stream_cosine_angles_by_layer: NDArray[np.float64],
    number_of_layers: int,  # Loop bound
) -> NDArray[np.float64]:  # Returns the updated fpt_terms
    # Clone fpt_terms because JAX/Numba loops often work by updating a copy
    # or carrying state functionally. Here, it will be modified in-place within the loop.
    # Numba allows in-place modification of arrays passed as arguments in nopython mode.
    fpt_terms_updated = (
        initial_fpt_terms.copy()
    )  # Start with a copy so original is not modified directly

    # Numba-compatible reversed loop
    for layer in range(number_of_layers - 1, 0, -1):
        fpt_terms_updated[
            :, :, layer - 1
        ] = (  # This line is correct with explicit indices
            fpt_terms_updated[:, :, layer] * em2_terms[..., layer]
            + gg_terms[..., layer]
...
def calculate_flux(
    ...
    fpt_terms: NDArray[np.float64] = np.zeros_like(
    ...
    fpt_terms = _calculate_flux_loop_kernel(
        fpt_terms,
        tau,
        gg_terms,
        hh_terms,
