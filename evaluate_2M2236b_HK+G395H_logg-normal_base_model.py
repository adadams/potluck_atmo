...
if __name__ == "__main__":
    apollo_model_filepath: Path = (
        input_file_directory / "2M2236b_HK+G395H_logg-normal_apollo_model.nc"
    )
    ...
    model_spectrum_rescaled: xr.DataArray = xr.where(
        ...
        test_model_spectrum * 1.151963518,
        test_model_spectrum,
    )

    model_spectrum_rescaled: xr.DataArray = xr.where(
        model_spectrum_rescaled.wavelength > 4.1,
        model_spectrum_rescaled * 0.8821451802,
    ...
    figure.savefig(
        output_file_directory / "2M2236b_HK+G395H_logg-normal_model_comparison.pdf",
        bbox_inches="tight",
    )
